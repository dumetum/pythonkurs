...
def berechne_arbeitszeit(ereignisliste):
    ...
    letzter_ereignis_typ = "Ende"
    for ereignis in ereignisliste:
        if ( (ereignis[1] == "Start" and letzter_ereignis_typ != "Ende") or (ereignis[1] == "Ende" and letzter_ereignis_typ != "Start") ):
            raise Exception("Ungültige Reihenfolge von Ereignisse: ", ereignis)
        letzter_ereignis_typ = ereignis[1]
...
def berechne_arbeitszeit_pro_projekt(ereignisliste):
    ...
    for projekt, projekt_ereignis_liste in projekt_ereignis_list_dict.items():
        arbeitszeit_fuer_projekt = -1
        try:
            arbeitszeit_fuer_projekt = berechne_arbeitszeit(projekt_ereignis_liste)
        except Exception as e:
            print("Fehler bei der Berechnung der Arbeitszeit für Projekt: ", e)
