...
* Das Programm soll beendet werden, wenn der Anwender eine negative Zahl eingibt.
...
anzahl = 0
Gebe eine Zahl ein
so lange die eingegebene Zahl nicht kleiner als null:
...
* Aufgabenstellung: Der Anwender gibt eine a priori nicht bekannte Anzahl von Zeichenketten ein.
(Die Zeichenketten können beispielsweise auch aus einer Datei kommen).
...
* Da alle Zeichenketten verglichen werden müssen, funktioniert auch das Vorgehen bei der Durchschnittsberechnung nicht
(Addiere die eingegebenen Zahlen so lange die eingegebene Zahl nicht kleiner als 0).
